...
describe('basic tests', () => {
    test('basic markdown input string', () => {
        expect(ChatgptToLogseqSanitizer.sanitize("Hello **World**!")).toMatch("Hello **World**!");
    });
    test('basic markdown input string 2', () => {
        const input = 'This is some plain text.';
        const expectedOutput = 'This is some plain text.';
        expect(ChatgptToLogseqSanitizer.sanitize(input)).toMatch(expectedOutput);
    });
    test('empty input string', () => {
        expect(ChatgptToLogseqSanitizer.sanitize("")).toMatch("");
    });
});
describe('unicode support tests', () => {
    test('unicode input string', () => {
        expect(ChatgptToLogseqSanitizer.sanitize("Hello **世界**!")).toMatch("Hello **世界**!");
    });
});
